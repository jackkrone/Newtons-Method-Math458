...
for k = 1:length(x(:))
  guess = [x(k);y(k)];
  [roots{k},iterVec(k)] = newtonMod(funct,jacFunct,guess,tol);
end
...
xlabel('Real')
ylabel('Imaginary')
title({'Newton''s Method Root Map', '\fontsize{8}This graph shows the root found by Newton''s Method for the given initial guess'})
hold off
